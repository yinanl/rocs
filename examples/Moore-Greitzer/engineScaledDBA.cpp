 ...
 *  engineScaledDBA.cpp
 *
 *  Reach and stay control of a scaled ode model of Moore-Greitzer engine.
 *
...
struct mgode2 {
	...
	dx[0] = cx * (aH+H2*(x[0]-W)*(W2-(x[0]-W)*(x[0]-W)) - x[1]/s) + u[0];
	dx[1] = s*cy * (x[0] - u[1]*sqrt(x[1]/s));
    }
};
