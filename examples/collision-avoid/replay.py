...
egoname = '/traj_closedloop_ca_' + args.case
othername = '/traj_other_robot_' + args.case
egofile = dirpath+egoname+'.txt'
otherfile = dirpath + othername+'.txt'
x = []
u = []
xo = []
...
# ani.save(dirpath+'case'+args.case+'.mp4', writer=writer)
...
# ll_other = (xo[j, 0]+rdiag*np.cos(a0+xo[j, 2]),
#                  xo[j, 1]+rdiag*np.sin(a0+xo[j, 2]))
# show_other = patches.Rectangle(ll_other, width, height,
#                               angle=xo[j, 2]*180/math.pi,
#                               fill = True, ec='grey', fc='grey', alpha=0.7)
# ax.add_patch(show_ego)
# ax.add_patch(show_other)
# ax.add_patch(rshow_ego)

plt.savefig(dirpath+egoname+'.pdf')
