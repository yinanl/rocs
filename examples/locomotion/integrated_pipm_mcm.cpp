/**
 *  integrated_pipm_mcm.cpp
 *  
 *  One walking step from PIPM to MCM:
...
const double ds2 = 0.15;
const double dz2 = 0.9*z0;
const double zmin = 0.6*z0;
...
void reachability_control(rocs::abstraction<WalkStep> &abst1, const double s1[], const double z1[],
    ...
    RobustSetMCM Rq2(x2, v2, w2, z0, s2, z2);
    RobustSet Rq1(x1, v1, w1*w1, z0, Dx, s1, z1);
    
    tb = clock();
    /* Get the target set */
    std::function<bool(const rocs::ivec &)> in_target =
    	std::bind(&RobustSetMCM::in_robust_set, &Rq2, std::placeholders::_1);
...
int main()
{
    ...
    const std::string prefix = "integrated/pipm2mcm/data_integrate_pipm2mcm_q";
    ...
    /* Control values */
    double ulb1[] = {2}; // the first variable denotes mode
    double uub1[] = {4};
    double ulb2[] = {-3}; // the first variable denotes mode
    double uub2[] = {-1};
    ...
    WalkStep step2("semi2", tau, n, m, MCM, loc2);
    ...
    /* Save grids */
    rocs::matWriter wtr("integrated/pipm2mcm/data_grids_pipm2mcm.mat");
    wtr.open();
