...
j= 3;
plot(tsim, xts(:,2*j-1:2*j), 'LineWidth',LW)
...
for j= 1:size(x0,2)
    plot(xts(:,1+2*(j-1)), xts(:,2+2*(j-1)),'LineWidth',LW);
end
plot(x0(1,:), x0(2,:), 'o','MarkerFaceColor',cr, 'MarkerEdgeColor',cr)
