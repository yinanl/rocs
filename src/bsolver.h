...
namespace rocs {
    ...
    */
    ...
    /**
     * Construt the DBA struct: assign values to
     * - n: # of DBA states.
     ...
     */
    void construct_dba(HEAD *head,
		       int nAP, int nNodes, int q0,
		       std::vector<rocs::UintSmall> &acc,
		       std::vector<std::vector<rocs::UintSmall>> arrayM) {
	DBA *dba = &(head->dba);
	clock_t start_t,end_t;

	printf("Start to construct DBA.\n");
	start_t=clock();
	dba->k = nAP;
	dba->n = nNodes;
	dba->ini = q0;
	...
	end_t=clock();
	printf("Time used to construct DBA = %.4f\n",
	       (double)(end_t-start_t)/CLOCKS_PER_SEC);
    }
    

    /**
     * Read NTS transitions, and
     * perform a safety preprocessing to delete transitions to/from avoid states.
     */
    void safety_pre(HEAD *head, fts &nts)
    {
	NTS_PRE *pre;
	...
	int action,a,*outdeg;
	long long cnt;
	long long x1,x2,x,n0,n0_1,m0,m0_1,*p3,*p4;
     ...
     */
    template<typename S>
    void pre2post(HEAD *head, abstraction<S> &abst)
    {
    	NTS_PRE *pre;
    	NODE_PRE *nts_pre,*p1;
	...
	*/
     ...
     */
    void write_controller_to_txt(HEAD *head, const char *buffer) {
	FILE *fout;
	clock_t start_t,end_t;
    ...
    /**
     * Solve a buchi game on a product system:
     * 1) Construct a backward graph for NTS, and do safety processing.
     * 2) Read the specification as a DBA structure from a file.
     * 3) Convert the backward graph to a forward graph.
     * 4) Take product of the forward graph and the DBA.
     * 5) Solve the buchi game on the product.
     */
    template<typename S>
    void solve_buchigame_on_product(HEAD *head, abstraction<S> &abst) {
	safety_pre(head, abst._ts);
	pre2post(head, abst);
	post2product(head);
	buchi_and_controller(head);
    }
