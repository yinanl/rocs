...
void SPtree::refine_leaf(SPnode *x, SPnode *sp, short tag) {
	...
	if (sp->_left->_box.isin(x->_box)) {
	    ...
	    // int axis = 0;
	    // for (int i = 0; i < sp->_box.getdim(); ++i) {

	    // 	if (sp->_left->_box[i] != sp->_right->_box[i]) {
...
void SPtree::tagging() {
	...
	if (isleaf(current)) {
	    ...
	    if (current->_left->_tag == 0 && current->_right->_tag == 0)
		current->_tag = 0;
	    else if (current->_left->_tag == 1 && current->_right->_tag == 1)
		current->_tag = 1;
	    else
...
/* save to files */
void SPtree::write2mat_leaves(SPnode *node, const char* filename) {
    ...
    int nleaf = leafcount(node); //count rows of output matrices
    
    /* define outputs */
    int nx = node->_box.getdim();
    mxArray *matx = mxCreateDoubleMatrix(nleaf, 2 * nx, mxREAL);
    double *ptrx = mxGetPr(matx);
    
    int nu = node->_cntl.size();
	...
	if (isleaf(current)) { //write leaves
    ...
    if (matPutVariable(pmat, "pavings", matx) ||
	matPutVariable(pmat, "ctlr", matu) ||
	matPutVariable(pmat, "tag", mattag) != 0) {
	
	std::cout << "Error outputting variables" << std::endl;
        return;
    }
...
void SPtree::write2txt_leaves(SPnode *node, const char *filename) {
	...
	if (isleaf(current)) { //write leaves
		...
		txtfile << lower[i] << ' ' << upper[i] << ' ';

	    txtfile << " " << current->_tag << ' ';

	    for (int u = 0; u < node->_cntl.size(); ++ u)
		txtfile << ' ' << current->_cntl[u];
