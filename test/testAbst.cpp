...
int main()
{
	...
	std::cout << "# of nodes: " << abst._ts._nx << '\n';
	...
	for(size_t j = 0; j < na; ++j) {
		...
		for(size_t pp=abst._ts._ptrpre[k*na+j];
		    pp<sk+abst._ts._npre[k*na+j]; ++pp) {
	...
	for(size_t j = 0; j < na; ++j) {
		...
		for(size_t pp=abst._ts._ptrpost[k*na+j];
		    pp<sk+abst._ts._npost[k*na+j]; ++pp) {
